...
-   Does not call the `xdg-utils` scripts, but the presence of `xdg-utils` compatible `mimeinfo.cache` is required. A lot more concise than the original `xdg-open`. Retains the original `xdg-open` exit codes, where applicable, for compatibility.
